...
export function sourceChip(s) {
  return `<a class="ext-chip" href="${R}${s.url}" data-note="${s.url}"><span class="ext-icon" aria-hidden="true">${kindIcon[s.sourceType] || '↗'}</span>${esc(s.title)}</a>`;
}
...
<script>try{var t=localStorage.getItem('theme');if(t)document.documentElement.dataset.theme=t;var p=localStorage.getItem('prov');if(p)document.documentElement.dataset.prov=p;}catch(e){}</script>
...
<header class="topbar">
  ...
  <a class="brand" href="${R}index.html"><span class="brand-mark" aria-hidden="true">🌿</span>${esc(site.siteTitle)}</a>
  ...
  <button class="icon-btn prov-btn" type="button" title="Highlight what's the author's vs sourced" aria-pressed="false">◐<span class="sr">Toggle provenance highlighting</span></button>
...
export function notePage(n, { rendered, mapThumb }) {
  ...
  const inspired = n.createdFrom.length || n.footer.createdFromUrls.length
    ? `<p class="inspired"><span class="lbl">Inspired by</span> ${n.createdFrom.map(sourceChip).join(' ')}${n.footer.createdFromUrls.map(u => `<a class="ext-chip" href="${esc(u)}" target="_blank" rel="noopener"><span class="ext-icon">↗</span>${esc(new URL(u).hostname.replace(/^www\./, ''))}</a>`).join(' ')}</p>` : '';
...
<article class="note">
  ...
  <header class="note-head">
    <h1>${esc(n.name)}</h1>
    <div class="meta">${badge(n.maturity)}${n.core ? '' : '<span class="pill further-pill" title="Connected to the interviewing notes, but not part of the main path">further reading</span>'}${created ? `<span class="muted">Started ${created}</span>` : ''}<span class="muted">${minutes} min read</span>${tags}</div>
    ${inspired}${compare}
  </header>
...
<article class="note source">
  ...
  <header class="src-head">
    ...
    <div>
      <div class="src-kind"><span aria-hidden="true">${kindIcon[s.sourceType] || '↗'}</span> ${kindLabel[s.sourceType] || s.sourceType}</div>
      <h1>${esc(s.title)}</h1>
...
// ---------- Home ----------
export function homePage({ site, entry, startHere, mocHtml, counts, legend, randomHtml }) {
  return `
<section class="hero">
  <p class="eyebrow">Shared notes</p>
  <h1>${esc(site.siteTitle)}</h1>
  <p class="lede">${esc(site.siteSubtitle)}. Notes on system design, coding and culture interviews, and how AI is changing all of it. ${counts.notes} notes, ${counts.sources} sources.</p>
</section>
<section class="legend" aria-label="How to read these notes">${legend}</section>
<section class="start">
  <h2 class="sec-title">Start here</h2>
  <ul class="start-grid">${startHere.map(n => `<li><a class="start-card" href="${R}${n.url}" data-note="${n.url}"><span class="start-title">${esc(n.name)}</span>${badge(n.maturity, { compact: true })}<span class="start-ex">${esc(n.excerpt || '')}</span><span class="start-meta">${n.treeChildren.length ? `${n.treeChildren.length} notes inside` : ''}</span></a></li>`).join('')}</ul>
</section>
${randomHtml || ''}
<section class="tools">
...
export const LEGEND = `
<div class="lg-item"><span class="lg-swatch own" aria-hidden="true"></span><div><strong>Plain text is mine:</strong> my own understanding and experience.</div></div>
<div class="lg-item"><span class="lg-swatch ext" aria-hidden="true"></span><div><strong>Framed blocks are sourced:</strong> quotes or paraphrases, with a link to where they come from.</div></div>
<div class="lg-item"><span class="lg-badges" aria-hidden="true">🌱🌿🌳</span><div><strong>Maturity:</strong> how fleshed-out a note is. “est.” means estimated automatically.</div></div>`;
